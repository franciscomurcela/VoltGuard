...
\IfFileExists{voltverde.png}{\includegraphics[width=0.35\linewidth]{voltverde.png}}{}
...
\textbf{Detect} — Automatic anomaly detection using time-series forecasting (Prophet) and outlier scoring (PyOD), with no manual threshold configuration. \\
...
VoltGuard follows a microservices architecture where each functional domain is isolated in its own deployable unit. All external traffic enters through a single ingress point: the Kong API Gateway. Then is forwarded to the appropriate backend service. 
...
\midrule
Compositor Frontend  & React + Vite       & Component-driven SPA with fast HMR for development \\
Compositor Backend   & Node.js / Express  & Lightweight SOA gateway; rich ecosystem for HTTP proxying \\
OAM Service          & Rust / Axum / SQLx & Memory safety and throughput for sensor inventory operations \\
Anomaly Detection    & Python / FastAPI   & Native ecosystem for ML libraries (Prophet, PyOD) \\
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Compositor Backend → Peers}: Proxied REST calls with configurable timeout, retry count, and backoff.
...
All endpoints are protected by Keycloak JWT validation. The backend verifies tokens against the Keycloak realm and rejects unauthenticated requests with HTTP 401. Sensitive configuration (client secrets, session keys, auth tokens) is loaded from Kubernetes secrets at runtime, its never from plain environment variables in the deployment manifest.
...
\begin{itemize}
  \item \textbf{Prophet} (Meta / Facebook) - a decomposable time-series forecasting model that captures trend and seasonality. Given a sensor's historical readings, Prophet produces a confidence interval for the expected next value. Readings outside the interval are flagged as anomalies.

  \item \textbf{PyOD} - a Python library for outlier detection. Used as a secondary scorer to catch point anomalies that fall within Prophet's interval but are statistically unusual relative to the sensor's recent distribution.
\end{itemize}

A reading is escalated to the Notifications service only if \emph{both} models agree it is anomalous, reducing false-positive noise.
...
\begin{itemize}[noitemsep]
  \item \texttt{POST /ingest} — receive a batch of sensor readings.
...
The Notifications service is a multi-channel alerting gateway. When the Anomaly Detection service confirms an anomaly, it calls this service to dispatch alerts through the configured channels.

\section{Technology}

The service follows an \textbf{OpenAPI-first} approach using \textbf{Connexion}, a Python framework that generates the WSGI application directly from an \texttt{openapi.yaml} specification. 

\section{Supported Channels}

All channel integrations are handled through the \textbf{Twilio} API. Delivery is idempotent — duplicate anomaly events with the same identifier will not result in duplicate messages.

\begin{table}[H]
\centering
\caption{Supported notification channels}
\label{tab:notify-channels}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{l l}
\toprule
...
\midrule
\texttt{/}            & Compositor Frontend \\
\texttt{/api/*}       & Compositor Backend \\
\texttt{/auth/*}      & Keycloak \\
...
\midrule
\texttt{compositor-data}        & Compositor Backend (session data) \\
...
\textbf{Terminal 2 — watch pod states and kill a Kong pod:}
...
# In the same or a separate pane: terminate a Kong pod
kubectl delete pod <kong-pod-name> -n tenant-grupo4-egs-deti-ua-pt
\end{lstlisting}
...
\begin{enumerate}[noitemsep]
  ...
  \item The surviving Kong replica(s) continue serving requests.
